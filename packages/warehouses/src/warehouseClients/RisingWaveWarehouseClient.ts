...
const mapFieldType = (type: string): DimensionType => {
    switch (type) {
        case RisingWaveTypes.DECIMAL:
        case RisingWaveTypes.NUMERIC:
        case RisingWaveTypes.INTEGER:
        case RisingWaveTypes.MONEY:
        case RisingWaveTypes.SMALLSERIAL:
        case RisingWaveTypes.SERIAL:
        case RisingWaveTypes.SERIAL2:
        case RisingWaveTypes.SERIAL4:
        case RisingWaveTypes.SERIAL8:
        case RisingWaveTypes.BIGSERIAL:
        case RisingWaveTypes.INT2:
        case RisingWaveTypes.INT4:
        case RisingWaveTypes.INT8:
        case RisingWaveTypes.BIGINT:
        case RisingWaveTypes.SMALLINT:
        case RisingWaveTypes.FLOAT:
        case RisingWaveTypes.FLOAT4:
        case RisingWaveTypes.FLOAT8:
        case RisingWaveTypes.DOUBLE_PRECISION:
        case RisingWaveTypes.REAL:
            return DimensionType.NUMBER;
        case RisingWaveTypes.DATE:
            return DimensionType.DATE;
        case RisingWaveTypes.TIME:
        case RisingWaveTypes.TIME_TZ:
        case RisingWaveTypes.TIMESTAMP:
        case RisingWaveTypes.TIMESTAMP_TZ:
        case RisingWaveTypes.TIME_WITHOUT_TIME_ZONE:
        case RisingWaveTypes.TIMESTAMP_WITHOUT_TIME_ZONE:
            return DimensionType.TIMESTAMP;
        case RisingWaveTypes.BOOLEAN:
        case RisingWaveTypes.BOOL:
...
> extends WarehouseBaseClient<T> {
    ...
    async streamQuery(
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            pool.on('error', (err) => {
                console.error(`RisingWave pool error ${err.message}`);
                reject(err);
            });
            ...
            pool.on('connect', (_client: pg.PoolClient) => {
                ...
                _client.on('error', (err: Error) => {
                    console.error(
                        `RisingWave client connect error ${err.message}`,
                    );
                    reject(err);
                });
            });
            pool.connect((err, client, done) => {
                ...
                client.on('error', (e) => {
                    console.error(`RisingWave client error ${e.message}`);
                    reject(e);
                    done();
                });
                        ...
                        .pipe(
                            new Writable({
                                ...
                                write(
                                    ...
                                    streamCallback({
                                        fields: RisingWaveClient.convertQueryResultFields(
                                            chunk.fields,
                                        ),
                                        rows: [chunk.row],
                                    });
                ...
                if (options?.timezone) {
                    console.debug(
                        `Setting risingwave session timezone ${options?.timezone}`,
                    );
                    client
            ...
            .catch((e) => {
                throw new WarehouseQueryError(
                    `Error running risingwave query: ${e}`,
                );
            })
            .finally(() => {
                pool?.end().catch(() => {
                    console.info('Failed to end risingwave pool');
                });
            });
    }
    ...
    getMetricSql(sql: string, metric: Metric) {
        switch (metric.type) {
            ...
            case MetricType.PERCENTILE:
                return `PERCENTILE_CONT(${(metric.percentile ?? 50) / 100
...
export class RisingWaveWarehouseClient extends RisingWaveClient<CreateRisingWaveCredentials> {
    constructor(credentials: CreateRisingWaveCredentials) {
